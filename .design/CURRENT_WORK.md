...
**First post is published:** `content/posts/dissecting-freetoken/`,
"Dissecting FreeToken", live since 2026-09-18 without the Draft badge. It has
an intro paragraph, the problems-and-solutions table, a sentence on the
...
- The FreeToken post's `description`, `summary`, `tags` and `categories` are
  still empty. The home card shows only title, date and reading time, so
  nothing is visibly missing; filling them is his call.
...
- `archetypes/posts.md` — new-post front matter plus the markup gotchas, in a
  reminder comment that must be deleted before publishing.
- `.github/workflows/deploy.yml` — build + deploy, Hugo pinned to 0.165.0.

## Key context

- Joon dictates tables as Markdown files on ml-engine, under
  `~/projects/freetoken-from-scratch/temp/` for the FreeToken post. Fetch them
  with `ssh ml-engine.local 'cat <path>'` and paste verbatim; those files can
  carry internal labels (milestone numbers, tool names) that he may want
  stripped — ask rather than edit.
- The FreeToken problems-and-solutions table came from the paper-reading
  session in `~/projects/paper-reads/freetoken`. That session cannot message
